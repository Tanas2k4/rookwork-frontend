...
export default function SettingsPage() {
  ...
  useEffect(() => {
    userApi
      ...
      .catch((err) => {
        console.error(err);
        addToast("Failed to load user data. Please log in again.", "error");
        setTimeout(() => {
          localStorage.clear();
            ...
            {TABS.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center px-3 py-2.5 text-sm rounded-lg transition-colors ${isActive ? "bg-purple-50 text-purple-700" : "text-gray-700 hover:bg-gray-100 hover:text-gray-900"}`}
                >
